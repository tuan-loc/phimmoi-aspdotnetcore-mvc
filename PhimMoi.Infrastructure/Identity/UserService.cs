...
namespace PhimMoi.Infrastructure.Identity
{
    public class UserService : IUserService
    {
        ...
        public UserService(IUnitOfWork unitOfWork, UserManager<User> userManager, SignInManager<User> signInManager)
        {
        ...
        public async Task<Result> DeleteAsync(string userId)
        {
            ...
            if (user == null) return Result.Error("User not found");
            IEnumerable<Comment> comments = await _unitOfWork.CommentRepository.GetAsync(c => c.User.Id == userId, includes: new System.Linq.Expressions.Expression<Func<Comment, object?>>[]
            {
                c => c.Responses
            });
        ...
        public async Task<PagedList<User>> SearchAsync(PagingParameter pagingParameter, string? value = null, string? role = null)
        {
            ...
            return await PagedList<User>.ToPagedListAsync(users, pagingParameter.Page, pagingParameter.Size, pagingParameter.AllowCalculateCount);
        }
        ...
        public async Task<Result> ToggleLockUserAsync(string userId)
        {
            ...
            {
                result = await _userManager.SetLockoutEndDateAsync(user, new DateTime(2023, 06, 08));
            }
            else
            {
        ...
        {
            return await _unitOfWork.UserRepository.FirstOrDefaultAsync(u => u.Id == id, new Expression<Func<User, object?>>[]
            {
                u => u.LikedMovies
            });
        }
