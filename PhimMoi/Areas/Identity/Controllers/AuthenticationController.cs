...
namespace PhimMoi.Areas.Identity.Controllers
{
    ...
    public class AuthenticationController : Controller
    {
        ...
        public AuthenticationController(IUserService userService, IAuthenticationService authenticationService, IEmailSender emailSender)
        {
        ...
        [HttpPost("/register")]
        public async Task<IActionResult> Register([FromForm] RegisterModel model, [FromForm] string? returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Success)
                {
                    ...
                    await _emailSender.SendEmailAsync(result.User.Email, "Xác thực Email", $"Hello người mới!, click vào <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>đây</a> để xác thực Email của bạn nhé :)");
        ...
        public async Task<IActionResult> ForgotPassword([FromForm] ForgotPasswordModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var callbackUrl = Url.Action("ResetPassword", "Authentication", values: new { area = "Identity", code }, protocol: Request.Scheme);
                _emailSender.SendEmailAsync(model.Email, "Reset Password", $"<a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>Ấn vào đây</a> để đặt lại mật khẩu của bạn.");
                return RedirectToAction("ForgotPasswordConfirmation");
            }
